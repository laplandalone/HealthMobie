...
<script type="text/javascript"
	src="pub/dialog/lhgdialog.min.js?skin=idialog"></script>
		...
		<div style="height:40px;margin-left:10px;line-height:40px;width:800px">
			<table width="100%">
				<tr>
					...
					<td><input type="text" id="medName" value="" style="150px" />
					</td>
				...
				<tr>
					...
					<c:choose>
							<c:when test="${userFiles.imgUrl0!=null && userFiles.imgUrl0!=''}">
								<a href="<c:out value="${userFiles.imgUrl0 }"/>" target="_blank"><img
									src="<c:out value="${userFiles.imgUrl0}"/>" width="60"
									height="60" />
								</a>
							</c:when>
					 ...
					 <c:choose>
							<c:when test="${userFiles.imgUrl1!=null && userFiles.imgUrl1!=''}">
								<a href="<c:out value="${userFiles.imgUrl1 }"/>" target="_blank"><img
									src="<c:out value="${userFiles.imgUrl1 }"/>" width="60"
									height="60" />
								</a>
							</c:when>
							...
							<c:when test="${userFiles.imgUrl2!=null && userFiles.imgUrl2!=''}">
								<a href="<c:out value="${userFiles.imgUrl2 }"/>" target="_blank"><img
									src="<c:out value="${userFiles.imgUrl2 }"/>" width="60"
									height="60" />
								</a>
							</c:when>

						</c:choose></td>
					<td align="center"><c:out value="${userFiles.storeName }"></c:out>
					</td>
					<td align="center">${userFiles.state}</td>
					<td align="center"><fmt:formatDate
							value="${userFiles.registerDate}" pattern="yyyy-MM-dd" />
					</td>
					...
					<td style="text-align:center !important"><c:choose>
							<c:when test="${userFiles.state == '未处理'}">
								<a href="javascript:void(0)" class="linkmore"
									onclick="verifyFunc('${userFiles.medicalName}',${userFiles.fileId})">审批</a>
							</c:when>
							<c:otherwise>
