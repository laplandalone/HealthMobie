				...
				else
				{
					$.ajax({
						...
						success:function(data)
						{
							if(data)
							{
								W.$.dialog({parent:api, title:false, width:"150px", esc:false, height:"60px", zIndex:2000, icon:'succ.png', lock:true, content:"成功修改回复信息!", ok:function() {renew(id); window.location.reload(); return true;}});
							}
							else
							{
								W.$.dialog({parent:api, title:false, width:"150px", esc:false, height:"60px", zIndex:2000, icon:'fail.png', lock:true, content:"修改回复信息失败!", ok:function() {$("#content" + id).val(oldContent); renew(id); window.location.reload(); return true;}});
							}
						},
			...
			<c:forEach items="${quesList }" var="ques">
				<tr>
					<td style='word-break:break-all;'>
						<fieldset style='width:350px; margin-top: 4px auto; margin-bottom: 10px; text-align: left;'>
							...
							<c:if test="${ques.recordType == 'ans' }">
								...
								<img src='/images/ans.png' height='25' width='30' style='vertical-align: top;margin-top: 5px'/>&nbsp;&nbsp;<textarea id='content${ques.id }' rows='4' cols='47' style='border: 0;height:50px;overflow:auto;margin-top: 5px'>${ques.content }</textarea>
							</c:if>
