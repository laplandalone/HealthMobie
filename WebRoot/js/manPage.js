function queryFunc() 
{
	...
	if (merOrderId.val() == "" && platOrderFlow.val() == ""
			&& platOrderId.val() == "") 
	{
		$.dialog.alert("请输入要查询的信息", function(){return true;});
	}
	else 
	{
		$("#queryOrder").submit();
	...
	if (tranOrderId != "" && bankCode != "" && outTradeNo != "") 
	{
		...
		$.ajax({
			...
			failure : function(result) 
			{
				...
				$.dialog.alert(result, function(){return true;});
			}
		})
	} 
	else 
	{
		$.dialog.alert("订单信息有误，请联系平台管理人员", function(){return true;});
	}
}